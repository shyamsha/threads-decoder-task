...
class Register extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    const passwordReg = true;
    // /(?=^.{8,15}$)(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#$%^&amp;*()_+}{&quot;:;'?/&gt;.&lt;,])(?!.*\s).*$/.test(
    //   this.state.password
    // );
    const usernameReg =
      /^([a-zA-Z]+)[0-9]*\.*[a-zA-Z0-9]+$|^[a-zA-Z]+[0-9]*$/.test(
        this.state.username
      );
    if (!usernameReg && !emailReg && !passwordReg) {
      this.setState(() => ({
        nameError: "Write Your Name > 3 Characters",
        nError: true,
        emailError: "Write Your Valid Email Format",
        eError: true,
        passwordError:
          "Must be alphanumeric with at least one number, one letter and capital letter, one special Character and be between 8-15 character in length.",
        pError: true,
      }));
    } else if (!usernameReg) {
      this.setState(() => ({
      ...
      axios
        ...
        .then((response) => {
          ...
          if (valid === "MongoError") {
            this.setState(() => ({ signupError: "User already registered" }));
